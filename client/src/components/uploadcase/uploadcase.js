...
const Uploadcase = () => {
  ...
  const onFinish = async (values) => {//点击提交事件，最后结果为result
   ...
   let res =  await window.contract.methods.postTask(result.title,allinfo.hazardType,result.content,imgSrc[0]).send( {
    ...
    if(res.transactionHash){
      ...
      setTimeout(()=>{
        ...
        message.error("抱歉，提交错误请稍后重试",3)
    }
  };
    return (
        <div className='uploadcase_bg'>
<ConfigProvider locale={zhCN}>
<div className='uploadcase'>
           <div className='uploadcase_all'>
