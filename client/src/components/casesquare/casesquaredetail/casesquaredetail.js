    ...
    return (
        <div>
            ...
            <div className='buttonlist'>
                ...
                <Button type='primary' size='large' className='assistan' onClick={Assistan}>请求协助</Button>
                <Modal
                  title={type === "audit"?"审核案件":"我要求助"}
                  visible={visible}
                  onOk={handleOk}
                  ...
                  cancelText="取消"
                >
                  <div>{type === "audit"?"审核意见":"求助描述"}<TextArea ref={input2} showCount  rows={4} placeholder="最多输入两百字" maxLength={200} /></div>
                </Modal>
            </div>
        </div>
    );
